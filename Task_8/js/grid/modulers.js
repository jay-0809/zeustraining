 ...
 */
export function inputField(grid, val = "") {
    const cellSelector = new CellSelector({ grid: grid });
    let globalCol = grid.multiCursor.col;
    let globalRow = grid.multiCursor.row;
...
export function keyNavigation(e, grid) {
    ...
    if (grid.multiEditing) {
        const range = grid.multiSelect;
        let { row, col } = grid.multiCursor;
        ...
        if (e.key === "Tab") {
            ...
            // console.log(e.key);
            inputField(grid, e.key);
            return;
        } else if (e.key === "ArrowUp") {
            grid.multiEditing = false;
        } else if (e.key === "ArrowDown") {
            grid.multiEditing = false;
        } else if (e.key === "ArrowLeft") {
            grid.multiEditing = false;
        ...
        if (handled) {
            grid.multiCursor.col = globalCol;
            grid.multiCursor.row = globalRow;
